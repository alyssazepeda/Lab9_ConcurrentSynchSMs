 ...
 *	Exercise Description:  https://youtu.be/botLth1lAHo
 *
...
const unsigned long periodIncDec = 20; //to account for button bouncing
/////////////////////////////////////////////////////////////
//State Machines
...
//The High Low Count which will vary as we inc/dec freq but will stay 50/50
//to allow for loudest output
...
enum IncDec_States{ID_Start, Inc, Dec, Release};
int IncDecSM_Tick(int state) {
	switch(state) {
		case ID_Start: 
			if((~PINA&0x03) == 0x01) {
				state = Inc;
	...
	switch(state) {
		case Inc: if(hlCount < 8) {hlCount++;} break; //just chose a random stopping point so theres a limit, after 8 it sounds about the same
		case Dec: if(hlCount > 1) {hlCount--;} break;
		default: break;
	}
	return state;
}	
